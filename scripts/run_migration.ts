...
async function runMigration(): Promise<boolean> {
  try {
    ...
    // Read migration file
    const migrationPath = path.join(__dirname, '..', 'migrations', '0003_update_client_measurement_relationship.sql');
    ...
    const indices = await client.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_measurements_client_id_unique';");
