...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  const isInSignupFlow = () => {
    return router?.currentRoute?.value?.path && SIGNUP_FLOW_PATHS.includes(router.currentRoute.value.path);
  };
  ...
  const checkSession = () => {
    ...
    if (isInSignupFlow() && timeUntilExpiry <= SESSION_TIMEOUT_WARNING && !sessionTimeoutWarningTimer) {
