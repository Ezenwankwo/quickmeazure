<template>
  <section id="pricing" class="py-[3rem] bg-[#F9FAFB] rounded-xl p-[2rem]">
    ...
    <p class="text-center text-[#4B5563] mb-[2rem] max-w-[36rem] mx-auto">
      ...
      {{ isAnnual ? 'Save 15% with annual billing.' : 'Start with our free plan and upgrade as your business grows.' }}
    </p>
    ...
    <div class="flex justify-center items-center gap-4 mb-[2rem]">
      ...
      <span v-if="isAnnual" class="text-sm bg-primary-100 text-primary-800 py-1 px-2 rounded-full">Save 15%</span>
    </div>
      ...
      >
        ...
        <div class="p-[1.5rem]">
          ...
          <div class="text-[2.25rem] font-bold mb-[1.5rem]">
            ...
            <span class="text-[1.125rem] text-[#6B7280] font-normal">/{{ formatInterval(plan.interval) }}</span>
          </div>
          
          <ul class="space-y-[0.75rem] mb-[1.5rem]">
            <li v-if="plan.maxClients !== undefined && plan.maxClients !== null" class="flex items-center">
