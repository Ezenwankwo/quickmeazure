<template>
  <div class="space-y-6">
        ...
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600">
              <template v-if="order.status === 'Pending'">Order is waiting to be processed</template>
              <template v-else-if="order.status === 'In Progress'">Order is currently being worked on</template>
              <template v-else-if="order.status === 'Ready for Pickup'">Order is ready for the client to pick up</template>
              <template v-else-if="order.status === 'Completed'">Order has been completed and delivered</template>
              <template v-else-if="order.status === 'Cancelled'">Order has been cancelled</template>
            </p>
          </div>
        </div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center">
              <div class="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center mr-3">
                <span class="text-primary-700 font-medium text-lg">{{ getInitials(order.clientName) }}</span>
              </div>
              <div>
                <NuxtLink :to="`/clients/${order.clientId}`" class="font-medium text-primary-600 hover:underline">
                  {{ order.clientName }}
                </NuxtLink>
            ...
            <div v-if="order.styleId" class="border-t border-gray-100 pt-4">
              ...
              <div class="flex items-center">
                <div v-if="order.styleImageUrl" class="w-16 h-16 bg-gray-100 rounded-md overflow-hidden mr-4">
                  <img :src="order.styleImageUrl" :alt="order.styleName" class="w-full h-full object-cover" />
                </div>
                <div>
                  <p class="font-medium">{{ order.styleName }}</p>
                  <NuxtLink :to="`/styles/${order.styleId}`" class="text-sm text-primary-600 hover:underline">
                    View Style Details
                  </NuxtLink>
                </div>
              </div>
            ...
            <div class="border-t border-gray-100 pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p :class="{
                  ...
                  <UBadge v-if="isOverdue(order.dueDate)" color="red" variant="subtle" size="xs" class="ml-2">
                    Overdue
                  </UBadge>
                  <UBadge v-else-if="isDueSoon(order.dueDate)" color="amber" variant="subtle" size="xs" class="ml-2">
                    Due Soon
                  </UBadge>
                </p>
              </div>
    ...
    <div v-else class="text-center py-12">
      ...
      <p class="mt-1 text-gray-500">The order you are looking for doesn't exist or you don't have permission to view it.</p>
      <div class="mt-6">
        <UButton to="/orders" color="primary">
          Go back to Orders
        </UButton>
      </div>
    </div>
