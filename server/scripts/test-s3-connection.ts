...
async function testS3Connection() {
  try {
    ...
    console.log(`- AWS_ENDPOINT_URL: ${process.env.AWS_ENDPOINT_URL || 'Not set (using default AWS endpoint)'}`);
    ...
    if (process.env.AWS_S3_BUCKET) {
      ...
      try {
        ...
        const baseUrl = process.env.AWS_ENDPOINT_URL || 
          (region === 'us-east-1' 
        ...
        if (error.$metadata?.httpStatusCode === 301) {
          ...
          if (redirectLocation) {
            ...
            const regionMatch = error.message?.match(/\b(us|eu|ap|ca|me|sa)-(east|west|central|north|south|northeast|southeast|southwest|northwest)-(1|2|3)\b/);
            if (regionMatch) {
